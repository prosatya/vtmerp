	...
	$.validator.addMethod("check_item_dropdown", function(value, element) {  
	 ...
	 $('#material').change(function () {
		 ...
		 var materialId = $(this).attr('value');
                    ...
                    $.ajax({   
                        url: "<?php echo base_url(); ?>ajax/productionMaterialOut/"+materialId, //The url where the server req would we made.
                        async: false,
...
<script type="text/javascript">
    ...
    <div id="contentwrapper">
       <div class="main_content">
         ...
         <li>
         <h3 style="color:#39c">Production Stock Out</h3>
         </li>
         </ul>
         </div>
        </nav>
	<?php    
     ...
     <?php
			$attributes = array('name' => 'materialmaster', 'id' => 'materialmaster');	   
	  		echo form_open('production/ProductionOutEdit' , $attributes);
      ...
      <table class="table table-bordered table-striped" id="smpl_tbl" style="border:none">
		...
		<select name="plant" id="plant"><option value="none">select plant</option>
         <?php $a=count($plants);
								if ($a>0){
								foreach($plants as $p){ ?>
         <option value="<?php echo $p['id']; ?>"<?php if($p['id'] == $fplantId['value']){ ?> selected <?php }?>><?php echo $p['plantName']; ?></option>
         <?php }} ?></select></div></td>
 ...
 <td><div class="row-fluid"><label>Material<span class="f_req">*</span></label>
		...
		<?php $a=count($materials);
         ...
         <option value="<?php echo $m['id']; ?>"<?php if($m['id'] == $fmaterialId['value']){ ?> selected <?php }?>><?php echo $m['material']; ?></option>
         <?php }} ?></select></div></td>
		...
		<td><div class="row-fluid"><label>Cost<span class="f_req">*</span></label>
			...
			<td><div class="row-fluid"><label>Stock Out<span class="f_req">*</span></label>
            <?php $atts = array(
				'name' => 'orderUnit',
			    'id'   => 'orderUnit',
			    'size' => 50,
			'value' => $fquantityOut['value'],
			   ); ?>
            <?php echo form_input($atts); ?>
            </div></td>
		<td><div class="row-fluid"><label>Balance<span class="f_req">*</span></label>
           <?php $atts = array(
			...
			<div class="displa_table">
					...
					echo $t; ?></td>                                        
<td><a href="<?php echo base_url(); ?>/index.php?production/productionOutEdit/<?php echo $row['stockId']; ?>" class="sepV_a" title="Edit"><i class="icon-pencil"></i></a>
